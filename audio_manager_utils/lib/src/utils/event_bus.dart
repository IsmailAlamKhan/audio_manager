...
extension ExtendedRef on Ref {
  void listenAndRefresh(List<Type> refreshEventTypes) {
    final eventBus = read(eventBusProvider);
    final sub = eventBus.onAll(
      (event) => invalidateSelf(),
      refreshEventTypes,
    );
...
mixin AutoRefreshAsyncNotifier<T> on AsyncNotifier<T> {
  void listenAndRefresh(List<Type> refreshEventTypes) => ref.listenAndRefresh(refreshEventTypes);
}
